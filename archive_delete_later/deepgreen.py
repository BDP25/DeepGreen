...
def segment_aoi(aoi, resolution, output):
    ...
    print(
        f"AOI has been split into Grid with Dimensions: {num_rows} x {num_cols} (rows x cols)"
    )
    ...
    for row in range(num_rows):
        ...
        for col in range(num_cols):
            ...
            row_AOIs.append(
                BBox(bbox=(min_x_new, min_y_new, max_x_new, max_y_new), crs=CRS.WGS84)
            )
            AOIs_flat.append(
                BBox(bbox=(min_x_new, min_y_new, max_x_new, max_y_new), crs=CRS.WGS84)
            )
...
def create_aoi_catalog(CATALOG, TIME_INTERVALL, AOI_BBOX_MAP, save_file=False):
    ...
    for AOI_ID in AOI_BBOX_MAP.keys():
        ...
        AOI_CATALOG_RESULTS = get_aoi_catalog_results(
            CATALOG, TIME_INTERVALL, AOI_ID, AOI_BBOX
        )
...
def filter_aoi_catalog_df(AOI_CATALOG_DF, MAXCC):
    ...
    print(
        f"> found {len(AOI_CATALOG_DF[(AOI_CATALOG_DF['CC']>MAXCC)])} entries to remove"
    )
...
def get_catalog_content(AOI, maxCC, start_date, end_date, access_token):
    ...
    data = {
        ...
        "bbox": AOI,
        "limit": 100,
        "filter": {"op": "<", "args": [{"property": "eo:cloud_cover"}, maxCC]},
    ...
    data = {
        "input": {
            ...
            "data": [
                {
                    "dataFilter": {
                        "timeRange": {"from": f"{time}", "to": f"{time}"},
...
def calculate_cc(AOI_CATALOG, AOI_BBOX_MAP, resolution, CONFIG):
    ...
    n_imgs = sum(
        len(keys)
        for keys in [
            list(AOI_CATALOG[primary_key].keys()) for primary_key in AOI_CATALOG
        ]
    )
    print(
        f"Estimated Time: {np.floor(n_imgs*time_per_img)} seconds, {(n_imgs*time_per_img)/60} minutes"
    )
