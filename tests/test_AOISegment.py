...
def mock_catalog_data():
    with open("mock_files/catalog_response.json") as f:
        return json.load(f)
...
@pytest.fixture
def mock_cloud_mask():
    return np.load("mock_files/test_mask_cloud.npy")


@pytest.fixture
def mock_red_mask():
    return np.load("mock_files/test_mask_red.npy")


@pytest.fixture
def mock_green_mask():
    return np.load("mock_files/test_mask_green.npy")


@pytest.fixture
def mock_blue_mask():
    return np.load("mock_files/test_mask_blue.npy")


@pytest.fixture
def mock_combined_mask():
    return np.load("mock_files/test_mask_combined.npy")


def test_get_time_stamps_and_api_cloud_coverage(mocker, mock_catalog_data, aoi_segment):
    mocker.patch.object(aoi_segment, "send_catalogue_request", return_value=iter(mock_catalog_data))
    ...
    for i, row in aoi_segment.df.iterrows():
        ...
        assert row["cloud_coverage_api"] == mock_catalog_data[i]["properties"]["eo:cloud_cover"]
...
def test_combine_masks(mock_cloud_mask, mock_red_mask, mock_green_mask, mock_blue_mask):
    combined_mask = AOISegment.combine_masks(mock_cloud_mask, mock_red_mask, mock_green_mask, mock_blue_mask)
